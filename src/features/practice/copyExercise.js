const DEFAULT_TARGET_TEXT = 'La práctica constante mejora la precisión, la velocidad y la confianza al escribir.';
...
export function createInitialState(targetText = DEFAULT_TARGET_TEXT) {
  return {
    targetText,
    typedText: '',
    errorLog: [],
...
export function validateCopy(targetText, typedText) {
...
export function updateStateWithInput(state, typedText, now = Date.now()) {
...
export function calculateFinalAccuracy(correctCount, registeredErrorCount) {
...
function renderCharacters(targetElement, characters) {
  targetElement.innerHTML = characters
    .map((character) => `<span class="char ${character.status}" data-index="${character.index}">${escapeHtml(character.expected)}</span>`)
    .join('');
}
...
function createErrorSound(windowRef) {
  let audioContext;
...
export function initialiseTypingExercise(container, { windowRef = window } = {}) {
  const elements = {
    targetText: container.querySelector('#target-text'),
    copyInput: container.querySelector('#copy-input'),
    soundEnabled: container.querySelector('#sound-enabled'),
    soundVolume: container.querySelector('#sound-volume'),
    resetButton: container.querySelector('#reset-button'),
    correctCount: container.querySelector('#correct-count'),
    errorCount: container.querySelector('#error-count'),
    pendingCount: container.querySelector('#pending-count'),
    accuracyRate: container.querySelector('#accuracy-rate'),
    completionMessage: container.querySelector('#completion-message'),
    errorLog: container.querySelector('#error-log'),
  };
  const abortController = new AbortController();
  const { signal } = abortController;
  const playErrorSound = createErrorSound(windowRef);
